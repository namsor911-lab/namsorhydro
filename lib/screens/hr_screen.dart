...
class _HrScreenState extends State<HrScreen> {
  ...
  Future<void> _exportCSV() async {
    ...
    try {
      ...
      final File file = File('${dir.path}/payroll_${_selectedYear}_${_selectedMonth}.csv');
      await file.writeAsString(csv.toString(), encoding: utf8);
      
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('✅ Export CSV ສຳເລັດ: ${file.path}'), backgroundColor: HrColors.green),
        );
      }
    } catch (e) {
      if (mounted) {
                ...
                children: <Widget>[
                  ...
                  Expanded(
                    child: _currentEmployees.isEmpty
                        ? Center(child: Text('ຍັງບໍ່ມີຂໍ້ມູນ — ກົດ "ເພີ່ມ" ເພື່ອເລີ່ມ', style: TextStyle(color: HrColors.txt3, fontSize: 16)))
                        : LayoutBuilder(
