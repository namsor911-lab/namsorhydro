...
class MonthlyRecord {
  final int month;
  final double outputKwh;   // Electrical Current Output (Kwh)
  final double importKwh;   // Electricity Imported For Use
  final double sellingPrice; // Selling Price ($) — ຄົງທີ່ 0.06
  double? expenses;          // Expenses ($) — user ປ້ອນໄດ້
...
class YearlyReportScreen extends StatefulWidget {
  /// ຂໍ້ມູນຈາກ load_screen._computeYearlyData()
...
class _YearlyReportScreenState extends State<YearlyReportScreen> {
  ...
  void initState() {
    ...
    _years = List.generate(10, (i) => now.year - 4 + i); // ± 5 ປີ
    _buildRecords();
    _initControllers();
  }
          ...
          children: [
            ...
            SafeArea(
              child: Column(
                ...
                children: [
                  // 1. ພາກສ່ວນເທິງ: Year Selector + Summary Cards ລຽນກັນແນວນອນ — ປັກໄວ້ຊາຍມືສຸດ
                  Align(
                  ...
                  // 2. ພາກສ່ວນລຸ່ມ: ຕາຕະລາງຊ້າຍ (60%) ແລະ ກຣາຟຂວາ (40%)
                  Expanded(
                    child: Padding(
  ...
  Widget _buildYearSelector() {
    return Container(
      ...
      child: Row(
        mainAxisSize: MainAxisSize.min, // ບັງຄັບບໍ່ໃຫ້ຍືດເຕັມຈໍ
  ...
  Widget _buildSummaryCards() {
    final cards = [
      (
        label: '⚡ Total Output',
        value: _fmt(_totalOutput),
        unit: 'Kwh',
        color: kGreen,
      ),
      (
        label: '⬇ Total Import',
        value: _fmt(_totalImport),
        unit: 'Kwh',
        color: kYellow,
      ),
      (
    ...
    return Container(
      ...
      child: Scrollbar(
        ...
        child: SingleChildScrollView(
          scrollDirection: Axis.horizontal,
          controller: _scrollCtrl,
          child: SingleChildScrollView( // ລວມທັງແນວຕັ້ງເພື່ອບໍ່ໃຫ້ລົ້ນຈໍຖ້າມີຫຼາຍແຖວ
